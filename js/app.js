...
if (iconMenu != null) {
  ...
  iconMenu.addEventListener("click", function (e) {
    if (unlock) {
      ...
      headerLogo.classList.toggle("logo__image_white");
    }
  });
}
function menu_close() {
...
let goto_links = document.querySelectorAll(".menu__link[data-goto]");
if (goto_links) {
...
function parallax(e) {
  document.querySelectorAll(".gragient-text").forEach((text) => {
    const speed = text.getAttribute("data-speed");
    const x = e.pageX / 20;
    text.style.backgroundPosition = x <= 100 ? `${x}%` : `${100}%`;
  });
}

document.addEventListener("mousemove", parallax);

// Add Anination while block in view on scroll

const hexagons = document.querySelectorAll(".item-feature__image");
...
let vacancies = document.querySelectorAll(".vacancies__item");
let isOneCollumn = window.innerWidth <= 768;
if (vacancies.length > 0) {
  let moreButton = document.querySelector(".showmore__button");
  let vacanciesList = document.querySelector(".vacancies__list");
  let vacanciesView = +vacanciesList.getAttribute("data-view");
  let vacanciesContentHideHeight = 0;
  let vacanciesContentFullHeight = vacanciesList.offsetHeight;

  addUnderLine(vacancies, vacanciesView, isEven(vacanciesView));

  for (let index = 0; index < vacanciesView; index++) {
    vacanciesContentHideHeight += vacancies[index].offsetHeight;
  }
  if (window.innerWidth <= 768) {
    vacanciesList.style.height = `${vacanciesContentHideHeight}px`;
  } else {
    if (isEven(vacanciesView)) {
      vacanciesList.style.height = `${vacanciesContentHideHeight / 2}px`;
    } else {
      let itemHeight = vacancies[0].offsetHeight;
      vacanciesList.style.height = `${vacanciesContentHideHeight / 2 + itemHeight / 2}px`;
      vacancies[vacanciesView].style.display = "none";
    }
  }
  moreButton.addEventListener("click", function (e) {
    e.preventDefault();
    addUnderLine(vacancies, vacancies.length, isEven(vacanciesView));
    vacanciesList.style.height = `${vacanciesContentFullHeight}px`;
